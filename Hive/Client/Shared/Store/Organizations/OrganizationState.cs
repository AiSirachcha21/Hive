...
{
    public record OrganizationState(bool IsLoading, IList<OrganizationViewModel> Organizations);

    public class OrganizationFeatureState : Feature<OrganizationState>
    {
        public override string GetName() => nameof(OrganizationState);

        protected override OrganizationState GetInitialState() => new(false, new List<OrganizationViewModel>(0));
    }
}
